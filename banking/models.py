...
class BankStatement(models.Model):
    ...
    def computed_balance(self):
        ...
        الرصيد الافتتاحي + مجموع الحركات
        """
        lines_sum = self.lines.aggregate(
            total=Sum("amount")
...
class BankStatementLine(models.Model):
    ...
    label = models.CharField(
        max_length=255,
        verbose_name=_("البيان/الوصف"),
    )
    ref = models.CharField(
        max_length=100,
        blank=True,
        verbose_name=_("المرجع البنكي"),
    )
    ...
    partner_name = models.CharField(
        ...
        blank=True,
        null=True,
        verbose_name=_("اسم الشريك المقترح"),
    )
    ...
    def clean(self):
        super().clean()
        if self.amount == 0:
            raise ValidationError(_("لا يمكن إدخال حركة بنكية بمبلغ صفر."))
    ...
    def save(self, *args, **kwargs):
        ...
        self.is_reconciled = (self.amount_residual == 0)
...
class BankReconciliation(models.Model):
    ...
    def clean(self):
        ...
        1. لا يمكن تسوية مبلغ صفر.
        2. لا يمكن تسوية مبلغ أكبر من المتبقي في سطر البنك.
        3. (لاحقاً) لا يمكن تسوية مبلغ أكبر من المتبقي في سطر القيد.
        """
        super().clean()

        if self.amount_reconciled == 0:
            raise ValidationError(_("لا يمكن إنشاء تسوية بمبلغ صفر."))

        if self.pk is None:
            # فحص سطر البنك: لا نتجاوز المتبقي
            if abs(self.amount_reconciled) > abs(self.bank_line.amount_residual):
                raise ValidationError(
                    _("المبلغ المراد تسويته أكبر من المبلغ المتبقي في سطر البنك.")
                )

            # لاحقاً: ممكن تضيف فحص على متبقي سطر القيد (JournalLine)
    ...
    def save(self, *args, **kwargs):
        ...
        # 2. الحفظ + تحديث المتبقي في سطر البنك ضمن معاملة واحدة
    ...
    def delete(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            # استخدم F لتقليل مخاطر الـ race condition
            BankStatementLine.objects.filter(pk=bank_line.pk).update(
                amount_residual=F("amount_residual") + amount_to_restore
            )

            # إعادة تحميل الحالة وتحديث is_reconciled عن طريق save()
            bank_line.refresh_from_db()
            bank_line.save()

    def _update_bank_line_residual(self):
        """
        تحديث المتبقي في سطر البنك بعد إنشاء تسوية جديدة.
        """
        BankStatementLine.objects.filter(pk=self.bank_line.pk).update(
            amount_residual=F("amount_residual") - self.amount_reconciled
        )
        self.bank_line.refresh_from_db()
        self.bank_line.save()

    def __str__(self):
        return f"Rec: {self.amount_reconciled} ({self.bank_line_id} <-> {self.journal_item_id})"
